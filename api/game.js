...
export default async function handler(req,res){
 ...
 try{
  const db=getDb();
  if(req.method==='GET'){
   const result=await db.select({nickname:scores.nickname,score:scores.score}).from(scores).orderBy(desc(scores.score),scores.createdAt).limit(25);
  ...
  if(body.action==='start'){
   ...
   await db.insert(runs).values({id,deck,startedAt:now,expiresAt:new Date(+now+90000)});
   return res.status(200).json({session:id,remainingMs:Math.max(0,+now+90000-Date.now()),question:publicQuestion(deck[0])});
  }
  if(!['answer','finish'].includes(body.action))throw fail(400,'Unknown action');
  ...
  const result=await db.transaction(async tx=>{
   ...
   if(body.action==='finish'){
    ...
    if(Date.now()<+run.expiresAt&&run.current<run.deck.length)throw fail(409,'The challenge is still running.');
    if(!run.submitted){
   ...
   if(Date.now()>=+run.expiresAt||run.submitted)return {expired:true};
   ...
   const current=run.current+1,score=run.score+(q.correct===body.choice?10:0);
   ...
   await tx.update(runs).set({current,score,lastAnswer:{choice:body.choice,response}}).where(eq(runs.id,run.id));
   return response;
  });
